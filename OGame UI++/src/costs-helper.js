var fn = function () {
    ...
    window._addCostsHelperInterval = function _addCostsHelperInterval() {
        ...
        function _addLimitingReagentHelper() {
            ...
            ['metal', 'crystal', 'deuterium'].forEach(function (res) {
                ...
                if (availableIn && availableIn > 0) {
                    if (availableIn > availableInLimitingReagent) {
                        limitingreagent[res] = true;
                        availableInLimitingReagent = availableIn;
                        gAvailableIn[res] = availableIn;
                    }
                }
            });

            for (let res in limitingreagent) {
                $('.' + res + '.tooltip.enhanced:not(.limitingreagent)').addClass('limitingreagent');
            }
        }
        ...
        function _addProductionBuildableInTextHelper() {
            ...
            let availableInMax = Math.max(gAvailableIn.metal, gAvailableIn.crystal, gAvailableIn.deuterium);
